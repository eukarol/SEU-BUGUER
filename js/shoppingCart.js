...
//configuração para redirecionar o cliente ao WhatsApp a loja (finalizar pedido)
...
const generateOrder = () => {
    ...
    // WhatsApp Number (replace with the actual number, without spaces or special characters)
    const whatsappNumber = '5587981579193'; // Substitua pelo número real da loja

    let cartMessage = 'Boa noite! Gostaria de fazer o pedido de: \n';
    ...
    const totalValue = allItemsValue + deliveryValue;
    const finalPrice = (totalValue - ((totalValue * discountValue) / 100)).toFixed(2);
    cartMessage += `\n\nValor Total: R$ ${finalPrice.replace('.', ',')}`;
